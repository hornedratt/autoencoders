...
def test_noise(input_path: str,
    ...
    col_len = len(original_profiles.columns)
    original_profiles = original_profiles.rename(columns={'group': '"group"', 'ID': '"ID"'})
    original_profiles = original_profiles.rename(columns={i: '"'+i+'"' for i in original_profiles.columns[:col_len-2]})
